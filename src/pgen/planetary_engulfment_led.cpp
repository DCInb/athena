...
void ConductiveRadiation(HydroDiffusion *phdif, MeshBlock *pmb,
...
bool IsPointInCell(Coordinates *pcoord, Real x, Real y, Real z, int k, int j, int i);
...
void Mesh::InitUserMeshData(ParameterInput *pin)
{
  ...
  // local vars
  Real rmin = pin->GetOrAddReal("mesh","x1min",5.7001e11);
  Real rmax = pin->GetOrAddReal("mesh","x1max",5.1301e12);
  ...
  EnrollConductionCoefficient(ConductiveRadiation);
  ...
  if(NSCALARS != 9){
    std::cout<<"COMPILED WITH "<<NSCALARS<<" SCALARS but 9 are required!!!";
  }
...
Real GetGM2factor(Real time, Real sep){
  ...
  // turn the gravity of the secondary off once reach inner boundary...
  if(sep>1.1*separation_stop_min){
    ...
    // if we're inside the inner boundary, turn the gravity of the secondary off
    GM2_factor = 0.0;
  }
...
Real kappa(Real rho, Real T)
{
  ...
  Real Kk = 4.e25*(1+X)*(Z)*rho*pow(T,-3.5);
  Real Khm = 1.1e-25*sqrt(Z) *sqrt(rho) * pow(T,1.7);
  Real Km = 0.1*Z;
  Real Krad = Km + 1.0/(1.0/Khm + 1.0/(Kes+Kk) );
  return Krad;
  // return mykappa;
}
...
{ 
  ...
  Real GM2_factor = GetGM2factor(time,d);
	...
	if(cooling){
	  ...
	  Real Teq = pow( Lstar/(4*PI*sigmaSB*r*r), 0.25); //fvir*GMenc1*mu*mH/(kB*r); // equilibrium temperature
    ...
    pmb->pscalars->s(8,k,j,i) = denr0*(ueq-u)*exp_step; // cooling radiation energy
...
void ConductiveRadiation(HydroDiffusion *phdif, MeshBlock *pmb,
  ...
  int is, int ie, int js, int je, int ks, int ke)
{
  // set the conduction coefficient equivalent to the radiation
...
Real getVolumeOfDeposition(Mesh *pm)
{
  ...
  // call MPI to do summation of deposition area volume
#ifdef MPI_PARALLEL
  // sum over all ranks
  if (Globals::my_rank == 0) {
    MPI_Reduce(MPI_IN_PLACE, &vol_dep, 1, MPI_ATHENA_REAL, MPI_SUM, 0,MPI_COMM_WORLD);
  } else {
    MPI_Reduce(&vol_dep,&vol_dep,1, MPI_ATHENA_REAL, MPI_SUM, 0,MPI_COMM_WORLD);
  }
  MPI_Bcast(&vol_dep,1,MPI_ATHENA_REAL,0,MPI_COMM_WORLD);
#endif
...
bool IsPointInCell(Coordinates *pcoord, Real x, Real y, Real z, int k, int j, int i) {
  Real r=pow((pow(x,2)+pow(y,2)+pow(z,2)),0.5);
  Real r_xy=pow((pow(x,2)+pow(y,2)),0.5);
  Real th=acos(z/r);
  Real ph=acos(x/r_xy);
  ...
  return (r >= pcoord->x1f(i) && r < pcoord->x1f(i+1)) &&
    (th >= pcoord->x2f(j) && th < pcoord->x2f(j+1)) && 
    (ph >= pcoord->x3f(k) && ph < pcoord->x3f(k+1));
}
  ...
  if (Globals::my_rank == 0 && time>t_relax){
    if(fixed_orbit){
      ...
      for (int ii=1; ii<=n_particle_substeps; ii++) {
        ...
        if (d<1.1*separation_stop_min) break;
...
Real Interpolate1DArrayEven(Real *x,Real *y,Real x0, int length){ 
  ...
  // supposing the x is uniform distribution （EVEN spacing)
