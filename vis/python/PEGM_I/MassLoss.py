...
def m_dot_parallel(i,myfile,phi_interp,gamma,rclist,m_dot_time,time,unbound=False):
    d = athdf(myfile)
...
def mass_loss(plot_num, plot_interv, rc_start, rc_num, rc_final, r_star,gamma, base_dir, unbound=False):
    """
    ...
    for i,myfile in enumerate(filelist):
        pool.apply_async(m_dot_parallel, args=(i,myfile,phi_interp,gamma,rclist,m_dot_time,time,unbound))
