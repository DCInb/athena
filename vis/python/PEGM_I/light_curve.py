...
def light_curve_radial_parallel(id_name, plot_interv, wavelength, time, int_lum, radii_aver, temp_aver,rho_aver,base_dir):
    """
    Temporal evolution of the integrated luminosity(parallel execution)
    ----
    Argments: tuple of (id_name, wavelength)
...
def light_curve_radial_parallel_plot(plot_num, plot_interv, wavelength,base_dir):
    ...
    pool = Pool(20)
    for i in np.arange(plot_num):
        pool.apply_async(light_curve_radial_parallel, args=(i,plot_interv,wavelength, time, int_lum, radii_aver, temp_aver,rho_aver,base_dir))
