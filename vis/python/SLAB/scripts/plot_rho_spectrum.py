...
def plot_Bk_t(file):
    ...
    k_bins = np.linspace(np.min(k), (np.max(k)), num=1000)  # Adjust bin size as needed
    ...
    n = int(num)
    ax.plot(bin_centers[1:], energy_spectrum[1:],label=str(n))
...
fig, ax = plt.subplots(1, 1, figsize=(5,4))  # 1 row, 2 columns
...
ax.set_xlabel(r'$|k|$')
ax.set_ylabel(r'$|\rho|^2(k)$')
ax.set_xscale('log')
ax.set_yscale('log')
ax.set_ylim(10**13,10**17)
ax.grid(True)
ax.legend()
...
plt.savefig(out_dir + '/rho_spectrum_t_1000r.pdf', format='pdf', bbox_inches='tight')